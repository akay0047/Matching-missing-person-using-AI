...
class AppWindow(QMainWindow):
    ...
    def initialize(self):
        ...
        button_match.resize(160, 60)
        button_match.move(570, 250)
        button_match.clicked.connect(self.match_from_submitted)

        submitted_case_button = QPushButton("View Found Cases", self)
        submitted_case_button.resize(160, 60)
        ...
        confirmed_case_button.resize(160, 60)
        confirmed_case_button.move(570, 450)
        confirmed_case_button.clicked.connect(self.view_found_cases)
    ...
    def view_found_cases(self):
        ...
        try:
            ...
            if cases == []:
                QMessageBox.about(self, "No cases Found", "No cases have been confirmed yet")
            else:
    ...
    def view_submitted_cases(self):
        ...
        try:
            cases = json.loads(requests.get(URL).text)
            if cases == []:
                QMessageBox.about(self, "No cases Found", "No cases have been submitted yet")
            else:
                self.view_submitted_cases_ui(cases)
        except requests.ConnectionError as e:
            QMessageBox.about(self, "Something went wrong", str(e))
    ...
    def view_submitted_cases_ui(self, result):
        ...
        for case_detail in result:
            ...
            item = QStandardItem(
                    "Name: " + case_detail[2] +
                    "\n Father's Name: " + case_detail[3] +
                    "\n Age: " + str(case_detail[4]) +
                    "\n Mobile: " + str(case_detail[5]) +
                    "\n Status: " + list(map(lambda x: 'Not Found' if x == 'NF' else 'Found', [case_detail[10]]))[0] +
                    "\n Submission Date: " + case_detail[8]
                    )
